...
export const FloatingMenuButton = React.forwardRef<HTMLButtonElement, FloatingMenuButtonProps>(
    ({ icon, variant = 'main', title, className, iconClassName = '', onClick, ...props }, ref) => {
        ...
        const variants = {
            main: "bg-foreground text-background border border-glass shadow-glass rounded-[1.25rem] w-[56px] h-[56px]",
            mainActive: "bg-surface-elevated text-foreground shadow-glow-strong border border-transparent rounded-full w-[56px] h-[56px]",
